...
if ! [ -z $CONTEST ]; then
    ...
    echo -e "#!/bin/bash \nexec python3 $EPSILON_PREFIX/manual_judge/judge.py \"$CONTEST_PATH\" -- \$@" > /usr/local/bin/judge
    echo -e "#!/bin/bash \nexec python3 $EPSILON_PREFIX/manual_judge/sh.py \"$CONTEST_PATH\" \$@" > /usr/local/bin/judgesh
    echo -e "#!/bin/bash \nexec python3 $EPSILON_PREFIX/judge/automatic-judge.py \"$CONTEST_PATH\" \$@" > /usr/local/bin/autojudge
    echo -e "#!/bin/bash \nexec python3 $EPSILON_PREFIX/server/epsilon.py -H 0.0.0.0 \"$CONTEST_PATH\" \$@" > /usr/local/bin/epsilon
else
